  ...
  for (const spec of SPECS) {
    ...
    for (const nusach of Object.keys(spec.sources) as NusachId[]) {
      ...
      for (const ref of refs) {
        ...
        const full = headingFor(ref, sel.prefix);
        ...
        seen.add(id);
        sections.push({ id, heading, ...(group ? { group } : {}), lines: res.lines });
      }
      if (sections.length) {
        text[nusach] = sections;
        available.push(nusach);
        const mod = `// Generated by scripts/build-siddur.ts — do not edit by hand.\nimport type { PrayerSection } from "../siddur";\n\nconst sections: PrayerSection[] = ${JSON.stringify(sections)};\n\nexport default sections;\n`;
        await writeFile(path.join(OUT, `${spec.id}.${nusach}.ts`), mod);
      }
    ...
    manifest.push({
      ...
      sections: Object.fromEntries(
        Object.entries(text).map(([k, v]) => [
          k,
          (v as { id: string; heading: string; group?: string }[]).map((s) => ({ id: s.id, heading: s.heading, group: s.group ?? null })),
        ]),
      ),
      licences: [...licences],
    });
