/**
 * Fitness discovery — server only.
 *
 * Venue data comes from Google Places (New) and travel times from the Routes
 * API, both through the Lovable connector gateway so no key ever reaches the
 * browser. Shekk's own commercial layer is merged on top in @/lib/fitness.
 */

import { extrasFor, type FitnessVenue } from "@/lib/fitness";
...
async function gateway<T>(path: string, init: RequestInit & { fieldMask?: string }): Promise<T> {
  if (!mapsConfigured()) {
    throw new Error("Fitness search needs the Google Maps connection — it isn't linked yet.");
  }
...
function toVenue(p: PlaceRow): FitnessVenue {
  ...
  return {
    ...
    ...(hours ? { hours } : {}),
    extras: extrasFor(name, types),
  };
}

/** Nearby fitness venues, optionally narrowed to one activity's Places types. */
export async function searchNearby(input: {
  ...
  const json = await gateway<{ places?: PlaceRow[] }>("/places/v1/places:searchNearby", {
    ...
    body: JSON.stringify({
      ...
      locationRestriction: {
        circle: { center: { latitude: input.lat, longitude: input.lon }, radius: Math.min(50_000, input.radiusM) },
      },
    }),
  });
  return (json.places ?? []).map(toVenue);
}

/** Free-text search — "pool in Ramat Beit Shemesh", "krav maga Tel Aviv". */
...
export async function placeDetails(placeId: string): Promise<FitnessVenue> {
...
export type TravelLeg = { mode: "WALK" | "TRANSIT" | "DRIVE"; minutes: number; km: number } | null;

/** One leg of travel from the member to the venue. Never throws — travel info is a nicety. */
export async function travelLeg(input: {
  ...
  try {
    ...
    console.error("travelLeg failed", e);
    return null;
  }
}
