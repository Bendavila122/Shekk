/**
 * The pages of the Shekk Passport: the front matter, the schematic map, and one
 * illustrated two-page spread per city.
 */
...
/** Shared two-page spread frame: left leaf, spine gutter, right leaf. */
function Spread({ left, right }: { left: React.ReactNode; right: React.ReactNode }) {
  return (
    <div className="grid h-full min-h-full grid-cols-2">
      <div className="relative min-w-0 overflow-hidden border-r border-ink/10 px-3 py-4">{left}</div>
      <div className="relative min-w-0 overflow-hidden px-3 py-4">{right}</div>
    </div>
  );
}
    ...
    <Spread
      left={
        <div className="flex h-full flex-col">
          <p className="text-[9px] font-bold uppercase tracking-[0.34em] text-ink/50">Shekk Passport</p>
          <h2 className="mt-2 font-display text-[26px] font-bold leading-[1.05] tracking-tight text-ink">
            Your Israel
            <br />
            Passport
          </h2>
          <Doodle className="mt-2 h-4 w-16 text-ink/30" />
          <p className="mt-3 text-[11px] leading-relaxed text-ink/70">
            Every city you reach gets a stamp and one photo. Fill the book by going places.
          </p>

          <dl className="mt-auto space-y-1.5 text-ink">
            <div>
              <dt className="text-[9px] font-bold uppercase tracking-[0.2em] text-ink/50">Holder</dt>
              <dd className="font-display text-base font-bold leading-tight">{name}</dd>
            </div>
            <div>
              <dt className="text-[9px] font-bold uppercase tracking-[0.2em] text-ink/50">Season</dt>
              <dd className="font-display text-base font-bold leading-tight">{seasonLabel(state.openedOn)}</dd>
            </div>
          </dl>
        </div>
      }
        ...
        <div className="flex h-full flex-col">
          <div className="grid grid-cols-2 gap-2">
            {[
              ...
              <div key={label} className="rounded-xl bg-ink/[0.045] px-2.5 py-2 ring-1 ring-ink/10">
                <Icon className="size-3.5 text-ink/50" />
                <p className="mt-1 font-display text-lg font-bold leading-none text-ink">{value}</p>
                <p className="text-[9px] font-semibold uppercase tracking-widest text-ink/50">{label}</p>
              </div>
            ))}
          </div>

          {/* progress: a row of little stamp slots that fill in */}
          ...
          <div className="mt-auto rounded-xl bg-ink/[0.045] p-2.5 ring-1 ring-ink/10">
            <p className="flex items-center gap-1.5 text-[10px] font-semibold text-ink/70">
              <Compass className="size-3.5" /> Flick sideways to turn the page
            </p>
          </div>
        </div>
      }
...
export function MapSpread({ state }: { state: PassportState }) {
  const visited = PASSPORT_CITIES.filter((c) => state.entries[c.id]?.visited);
  return (
    <Spread
      left={
        <div className="flex h-full flex-col">
          <p className="text-[9px] font-bold uppercase tracking-[0.34em] text-ink/50">The map</p>
          <h2 className="mt-1.5 font-display text-xl font-bold leading-tight tracking-tight text-ink">
            Where you've been
          </h2>
      ...
      right={
        <div className="flex h-full flex-col">
          <p className="text-[9px] font-bold uppercase tracking-[0.34em] text-ink/50">Log</p>
          {visited.length === 0 ? (
            <p className="mt-3 text-[11px] leading-relaxed text-ink/70">
              Nothing stamped yet. Turn to any city and check in when you get there — the book fills itself.
            </p>
          ) : (
            <ul className="mt-2 space-y-1.5">
              {visited.map((c) => (
                <li key={c.id} className="flex items-baseline gap-2">
                  ...
                  <span className="truncate font-display text-[13px] font-bold text-ink">{c.name}</span>
          ...
          <p className="mt-auto text-[10px] leading-relaxed text-ink/55">
            A hand-drawn schematic, not a survey map — city dots sit roughly where they belong.
          </p>
        </div>
      }
    />
...
export function CitySpread({
  ...
  return (
    <Spread
      left={
        <div className="flex h-full flex-col">
          <div className="relative -mx-3 -mt-4 h-[46%] overflow-hidden">
          ...
          <div className="mt-3">
            <p className="text-[9px] font-bold uppercase tracking-[0.3em]" style={{ color: city.ink }}>
              {city.hebrew}
            </p>
            <h2 className="font-display text-[22px] font-bold leading-none tracking-tight text-ink">{city.name}</h2>
            <Doodle className="mt-1.5 h-3.5 w-14" style={{ color: city.ink }} />
            <p className="mt-2 text-[11px] leading-relaxed text-ink/70">{city.blurb}</p>
          </div>
          ...
          {visited ? (
            <>
              <div className="mx-auto -mt-1">
                <PassportStamp city={city} date={entry?.visitedOn} animate={justStamped} size={120} />
              </div>
              <div className="mt-1">
            ...
            <div className="flex h-full flex-col">
              <p className="text-[9px] font-bold uppercase tracking-[0.3em] text-ink/50">Not stamped yet</p>
              <div
                className="mt-3 grid aspect-square w-full max-w-[9.5rem] place-items-center self-center rounded-full text-center"
              ...
              {checkInState.message ? (
                <p className="mt-2 text-center text-[10px] leading-relaxed text-ink/60">{checkInState.message}</p>
              ) : null}
              <p className="mt-auto text-[10px] leading-relaxed text-ink/45">
                “I'm here” checks roughly where you are. Location off? Mark it visited yourself.
              </p>
            </div>
          )}
        </div>
